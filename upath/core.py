...
class UPath(PathlibPathShim, Path):
    ...
    def __new__(
        ...
        *args,
        ...
        chain_parser: FSSpecChainParser = FSSpecChainParser(),
        **storage_options: Any,
    ) -> UPath:
        ...
        segments = chain_parser.unchain(str(part0), storage_options)
        chain = CurrentChainSegment.from_list(segments)
        if not chain.current.protocol == pth_protocol:
            warnings.warn(
                f"Unexpected protocol mismatch {chain.current.protocol!r} != {pth_protocol!r}",
                stacklevel=2,
            )
