...
class UPath(_UPathMixin, OpenablePath):
    ...
    def __vfspath__(self) -> str:
        if self._relative_base is not None:
            # Return relative path when _relative_base is set
            try:
                ...
                if full_path_no_protocol.startswith(root_path_no_protocol):
                    rel_path = full_path_no_protocol[
                        len(root_path_no_protocol) :
                    ].lstrip(self.parser.sep)
                    return rel_path or "."
                else:
                    # If paths don't have the expected relationship, fall back
                    return full_path
            except Exception:
                # If anything goes wrong, fall back to full path
